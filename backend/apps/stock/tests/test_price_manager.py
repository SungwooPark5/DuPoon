...
def test_get_latest_prices(stock, setup_prices):
...
@pytest.mark.django_db
def test_get_latest_price_date(stock, setup_prices):
    latest_date = Price.objects.get_latest_price_date(stock=stock)
    assert latest_date == date(2020, 1, 1)


@pytest.mark.django_db
def test_get_latest_price_date_returns_none_if_no_price(stock, setup_prices):
...
@pytest.mark.django_db
def test_get_adj_close_dataframe(stock, setup_prices):
